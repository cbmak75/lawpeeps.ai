/**
 * triage.mjs -- Batched Haiku triage stage
 *
 * Takes a pre-filtered shortlist of up to 20 candidates (already scored
 * by prefilter.mjs), sends them to Haiku in a single call, and gets back
 * a ranked JSON verdict. Top picks are enqueued for the editor.
 *
 * This replaces the per-item Sonnet + web_search calls in the old scout,
 * discover, and research stages. One Haiku call instead of 3-5 Sonnet
 * calls per cycle. Web search is reserved for verifier.mjs only.
 *
...
function loadRecentCoverage() {
  ...
  // Widen to the last 60 entries and pass slug, category and the
  // distinctive entities from the fingerprint. Haiku can then spot
  // "this is the MoJ AI transcripts story from two days ago under a
  // different headline" without needing any search.
  return (log.entries || []).slice(-60).map(e => ({
    title: e.title,
    slug: e.slug,
    category: e.category,
...
export async function runTriage({ tierFilter = null } = {}) {
  ...
  const shortlist = buildTriageShortlist(prefiltered, { maxItems: MAX_SHORTLIST, includeBorderline: true });
  if (shortlist.length === 0) {
    console.log('[triage] Prefilter rejected everything. Nothing to send to Haiku.');
  ...
  console.log(`[triage] Sending ${shortlistWithIds.length}-item shortlist to Haiku...`);
  ...
  try {
    ...
    console.error('[triage] Failed to parse Haiku response:', err.message);
  ...
  if (response.usage) {
    console.log(`[triage] Tokens: ${response.usage.input_tokens || '?'} in, ${response.usage.output_tokens || '?'} out, cache_read: ${response.usage.cache_read_input_tokens || 0}`);
  }
  console.log(`${'='.repeat(60)}\n`);
